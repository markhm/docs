...
tab-title: Usage
page-links: \
https://github.com/vaadin/vaadin-flow-components/releases/tag/{moduleMavenVersion:com.vaadin:vaadin-tabs-flow}[{moduleMavenVersion:com.vaadin:vaadin-tabs-flow}], \
...
// tag::description[]
Tabs are used to organise and group content into sections that the user can quickly navigate between.
// end::description[]
...
Disable a Tab to mark it as *currently unavailable*.
...
Choice of orientation should be made based on the use case and the available space.
...
The buttons indicate that there are more tabs to either side.
The buttons can be hidden by applying the `hide-scroll-buttons` theme variant.
...
* Use with caution; default left-alogned tabs are more easily discoverable.
...
For a very small number of tabs in a horizontally narrow space, such as:
...
[source, java]
----
include::{root}/src/main/java/com/vaadin/demo/component/tabs/TabsFocusRing.java[render,tags=snippet,indent=0,group=Java]
----
